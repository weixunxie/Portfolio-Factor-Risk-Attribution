...
- **SEC evidence** — risk evidence comes from 10-K filings ingested into Qdrant. ~82 common large-cap tickers are pre-ingested (see `data/universe.txt` + `scripts/bulk_ingest.py`). Any other ticker is **ingested on demand**: the first analysis that references it triggers a background extract+ingest and shows a "preparing…" note; re-running the analysis ~30–60s later surfaces the evidence (it is then cached in Qdrant permanently). A ticker yields no evidence only when its 10-K incorporates risk factors by reference (e.g. WFC) or it has no 10-K at all (ETFs, foreign issuers).
